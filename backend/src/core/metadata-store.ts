...
export class MetadataStore {
  public dag: LineageDAG;
  public tableSchemas: Map<string, string[]>;
  public tableMetadata: Map<string, TableMetadata>;
  ...
  async loadFromDb() {
    ...
    try {
      ...
      for (const point of points) {
        const payload = point.payload as TableMetadata | undefined;
        ...
        // 1. Hydrate Schemas
        const columns = column_metadata ? column_metadata.map(c => c.name) : [];
