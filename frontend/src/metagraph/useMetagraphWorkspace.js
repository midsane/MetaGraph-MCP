...
import { ACCENTS, INITIAL_SQL, NAV } from './constants.js';
import { request } from './api.js';
import { buildGraphData, buildSuggestions, getRiskHits, getStatementCount } from './utils.js';
...
export function useMetagraphWorkspace() {
    ...
    const loadWorkspace = useCallback(async () => {
        ...
        try {
            ...
            setCatalog(tables);
            setSelectedTable(current => (tables.some(table => table.tableName === current) ? current : (tables[0]?.tableName || '')));
    ...
    useEffect(() => {
        ...
        request(`/api/governance/${encodeURIComponent(selectedTable)}?role=${userRole}`)
            .then(data => {
                if (!cancelled) {
    ...
    const graphData = useMemo(() => buildGraphData(lineageData, catalog), [catalog, lineageData]);
    const activeNav = useMemo(() => NAV.find(tab => tab.id === activeTab), [activeTab]);
    ...
    const handleIngest = useCallback(async () => {
        ...
        try {
            ...
            setIngestLogs(`Ingestion complete. Registered ${data.tables?.length || 0} table(s).`);
            await loadWorkspace();
    ...
    const handlePurge = useCallback(async () => {
        if (!window.confirm('Purge all catalog metadata and lineage from Qdrant?')) {
            return;
        }
    ...
    const handleSearch = useCallback(async (event, query = ragQuery) => {
